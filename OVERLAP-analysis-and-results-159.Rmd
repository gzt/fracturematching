---
title: "Three image Classification"
...
knifematrix.partarray = array(t(knifematrix.part),dim = c(2,9,81))[,c(1,5,9),]
...
knifematrix.partarray = array(t(knifematrix.part),dim = c(2,9,100))[,c(1,5,9),]
matchingknives = knifematrix.partarray[,,matchlabels == "match"]
nonmatchingknives = knifematrix.partarray[,,matchlabels != "match"]
...
```{r othergraph, fig.cap = "The log-odds (base 10) for classifiers trained, using 3 images per surface, on each set of surfaces and tested on all sets of surfaces. Log-odds greater than 0 indicate classification as a match. ."}
...
#  ggtitle("Log-odds for models trained on each set of surfaces") +
  ...
  #                               "K-9-9-01", "S-10-9-10-01", "S-10-09-02-01")) +
    ...
    NULL
p
logoddsplotname = paste0("N-PNAS-log-odds-each-set-0-df-",df,".png")
...
This chunk provides testing on subsets of images (ie, $k = 2, 3$).
...
resultsstore = data.frame(model = c("1","K-9-9-01","S-10-9-10-01","S-10-09-02-01"),
                       fn2 = rep(0,2), fp2 = rep(0,2), fn3  = rep(0,2), fp3 = rep(0,2),
                       fn4 = rep(0,2),fp4 = rep(0,2),fn5 = rep(0,2),fp5 = rep(0,2),
                       fn6 = rep(0,2),fp6 = rep(0,2),fn7 = rep(0,2),fp7 = rep(0,2),
                       fn8 = rep(0,2),fp8 = rep(0,2),fn9 = rep(0,2),fp9 = rep(0,2))
...
for(k in 2:3){
chooseset = data.frame(lengthen(3,k))
...
```{r plotofstoredresults, fig.cap = "The log-odds (base 10) for classifiers trained, using 3 images per surface, on each set of surfaces and tested on each set of surfaces using a subset of the images. The classifiers were tested using all possible subsets of a specified size.", dev = "png"}

storeresultsdf$matching = storeresultsdf$match
storeresultsdf$k = factor(storeresultsdf$k, levels = 0:3)
 str(storeresultsdf)
...
#
#
#NPNASnumbersplot = paste0('N-NPNAS-numbers-per-image-0-df-',df,'.png')
...
# Plotting the worst case log odds
...
#allsetsname = paste0("NPNAS-0-sets-df-",df,".pdf")
#ggsave(allsetsname, plot = postlogplot, width = 4, height = 2.5)
...
resultsdfname = paste0('Nfinal-log-odds-0-df-',df,'.csv')
